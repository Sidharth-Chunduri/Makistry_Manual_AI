...
const FeatureTreeComponent: React.FC<FeatureTreeProps> = ({
  ...
  const loadFeatureTree = async () => {
    ...
    try {
      ...
      // Handle 404 as no feature tree exists yet
      if (err?.response?.status === 404) {
        setError('No feature tree exists for this project yet. Generate a design first to create a feature tree.');
      } else if (err?.response?.status === 500) {
        setError('Server error loading feature tree. This may indicate an issue with the backend.');
      } else {
