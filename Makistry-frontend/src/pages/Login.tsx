...
export default function Login() {
  ...
  // legacy support for your existing replay flow (kept intact)
  ...
  const handleEmailLink = async () => {
    try {
      setLoading(true);
      await sendMagicLink(email, nextPath);          // ← pass next
      setErr(null);
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-[#D6F3FF]">
      <div className="max-w-sm w-full p-6 bg-white rounded-xl shadow">
        ...
        <GoogleButton
          onSuccess={() => {
            // GoogleButton should complete auth -> JWT -> useAuth.login() internally.
            // After that, honor ?next=
        ...
        <Button
          className="w-full bg-[#FFCA85] text-black hover:bg-[#FFCA85]/80"
        ...
        <p className="mt-4 text-center text-sm">
          ...
          {/* Preserve ?next=… when switching to signup */}
          <Link to={`/signup${location.search}`} className="underline text-primary">
            Sign up
          </Link>
        </p>
      </div>
